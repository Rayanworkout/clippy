...
fn main() -> Result<()> {
    ...
    tracing::info!("Clippy listening for UI requests on {UI_LISTENING_PORT} ...");
    Arc::clone(&clippy).listen_for_ui();

    // Main thread
    tracing::info!("Clippy listening for clipboard changes and ready to send to UI on port {UI_SENDING_PORT} ...");
